...
if isolated_nodes:
    ...
    print("No isolates detected in LCC good.")


def stable_sort_communities(communities):

    #sort by size desc and then by smallest node id asc
    def key_fn(c):
        try:
            mn = min(c)
        except ValueError:
