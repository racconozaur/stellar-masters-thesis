...
TYPE = 'trustlines'
FN = f"{TYPE}/tl_node2vec_dbscan_cosine_pca64_kgrid_test.csv"
...
print(f"Found {len(db_cols)} DBSCAN label columns for intrinsic scoring.")
...
for i, col in enumerate(db_cols, 1):
    ...
    if core_size >= CORE_MIN and n_clusters >= 2:
        ...
        print("  Skipped intrinsic metrics (core too small or <2 clusters).")
...
print(f"Matched accounts for extrinsic eval: {len(df_ext):,} / {len(emb_all):,}")

db_cols_ext = [c for c in df_ext.columns if c.startswith("dbscan_")]
print(f"Found {len(db_cols_ext)} DBSCAN label columns for extrinsic scoring.")
...
for j, col in enumerate(sorted(db_cols_ext), 1):
    ...
    if core_size > 0 and n_clusters >= 1:
        ...
        print("  Skipped masked extrinsic (empty core).")
...
print("\nExtrinsic summary (first 5 rows):")
print(ext.head())
